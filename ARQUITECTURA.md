# 🏗️ ARQUITECTURA DE LA APLICACIÓN

```
╔════════════════════════════════════════════════════════════════════════════╗
║                  SISTEMA DE INVENTARIO LOCAL - ARQUITECTURA                ║
╚════════════════════════════════════════════════════════════════════════════╝

┌────────────────────────────────────────────────────────────────────────────┐
│                        CAPA DE INTERFAZ (interfaz.py)                      │
│                              GUI - tkinter                                 │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────┐      │
│  │                    VENTANA DE LOGIN                              │      │
│  │  Usuario: [________] Contraseña: [________] [Ingresar]          │      │
│  └─────────────────────────────────────────────────────────────────┘      │
│                                ↓                                            │
│  ┌─────────────────────────────────────────────────────────────────┐      │
│  │              APLICACIÓN PRINCIPAL (5 PESTAÑAS)                  │      │
│  ├─────────────────────────────────────────────────────────────────┤      │
│  │                                                                   │      │
│  │ [📦 Productos] [📥 Entradas] [📤 Salidas] [🧾 Facturas] [📊 Rep]│      │
│  │                                                                   │      │
│  │  ┌──────────────────────────────────────────────────────────┐   │      │
│  │  │  PESTAÑA ACTIVA (Contenido dinámico)                      │   │      │
│  │  │  - Formularios de entrada                                 │   │      │
│  │  │  - Tablas de datos                                        │   │      │
│  │  │  - Botones de acción                                      │   │      │
│  │  │                                                             │   │      │
│  │  └──────────────────────────────────────────────────────────┘   │      │
│  └─────────────────────────────────────────────────────────────────┘      │
│                                                                              │
└────────────────────────────────────────────────────────────────────────────┘
                                    ↓
┌────────────────────────────────────────────────────────────────────────────┐
│                      CAPA DE LÓGICA (logica.py)                            │
│                     Clase: LogicaInventario                                │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌─────────────┐   │
│  │ Autenticación│  │   Productos  │  │   Entradas   │  │   Salidas   │   │
│  │  - login()   │  │  - crear()   │  │  - registrar│  │  -registrar │   │
│  │  - logout()  │  │  - obtener() │  │  - obtener()│  │  -obtener() │   │
│  │- get_usuario │  │  - bajo_stock│  │             │  │             │   │
│  └──────────────┘  └──────────────┘  └──────────────┘  └─────────────┘   │
│                                                                              │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                    │
│  │    Facturas  │  │   Reportes   │  │  Validaciones│                    │
│  │  - crear()   │  │  - ventas()  │  │  - verificar │                    │
│  │  - agregar   │  │  - productos │  │  - calcular  │                    │
│  │  - obtener() │  │  - stats()   │  │             │                    │
│  └──────────────┘  └──────────────┘  └──────────────┘                    │
│                                                                              │
└────────────────────────────────────────────────────────────────────────────┘
                                    ↓
┌────────────────────────────────────────────────────────────────────────────┐
│                    CAPA DE DATOS (database.py)                             │
│                  Clase: DatabaseManager - SQLite                           │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌─────────────┐   │
│  │   USUARIOS   │  │   PRODUCTOS  │  │   ENTRADAS   │  │   SALIDAS   │   │
│  ├──────────────┤  ├──────────────┤  ├──────────────┤  ├─────────────┤   │
│  │ id (PK)      │  │ id (PK)      │  │ id (PK)      │  │ id (PK)     │   │
│  │ usuario      │  │ nombre       │  │ producto_id  │  │ producto_id │   │
│  │ contraseña   │  │ precio       │  │ cantidad     │  │ cantidad    │   │
│  │ nombre       │  │ stock        │  │ precio_unit  │  │ precio_unit │   │
│  │ email        │  │ stock_minimo │  │ proveedor    │  │ cliente     │   │
│  └──────────────┘  └──────────────┘  │ fecha        │  │ fecha       │   │
│                                      │ usuario_id   │  │ factura_id  │   │
│  ┌──────────────┐                    └──────────────┘  └─────────────┘   │
│  │   FACTURAS   │                                                          │
│  ├──────────────┤                     ┌──────────────────────────┐        │
│  │ id (PK)      │                     │  RELACIONES (FK)         │        │
│  │ numero       │                     ├──────────────────────────┤        │
│  │ fecha        │                     │ producto_id → productos  │        │
│  │ cliente_nombre                   n │ usuario_id → usuarios    │        │
│  │ cliente_nit  │                     │ factura_id → facturas    │        │
│  │ subtotal     │                     └──────────────────────────┘        │
│  │ iva_valor    │                                                          │
│  │ total        │                                                          │
│  │ estado       │                                                          │
│  └──────────────┘                                                          │
│                                                                              │
└────────────────────────────────────────────────────────────────────────────┘
                                    ↓
                    ╔════════════════════════════════════════╗
                    ║   inventario.db (SQLite)              ║
                    ║   - Almacenamiento persistente        ║
                    ║   - Compartible entre usuarios        ║
                    ║   - Sin servidor requerido            ║
                    ╚════════════════════════════════════════╝
```

---

## 📋 FLUJO DE DATOS

```
USUARIO INTERACTÚA CON INTERFAZ
            ↓
    INTERFAZ (interfaz.py)
    - Captura datos del usuario
    - Valida entrada básica
    - Llama métodos de LogicaInventario
            ↓
    LÓGICA (logica.py)
    - Valida reglas de negocio
    - Calcula valores
    - Coordina operaciones
    - Llama DatabaseManager
            ↓
    BASE DE DATOS (database.py)
    - Conecta a SQLite
    - Ejecuta queries
    - Retorna datos
            ↓
    INVENTARIO.DB
    - Almacena/Recupera datos
            ↓
    RESPUESTA REGRESA A INTERFAZ
    - Actualiza tablas
    - Muestra mensajes
    - Permite siguiente acción
```

---

## 🔄 EJEMPLO: GENERAR UNA FACTURA

```
1. USUARIO ENTRA EN PESTAÑA FACTURAS
   └─→ Interfaz muestra formulario

2. USUARIO COMPLETA DATOS:
   └─→ Nombre cliente: "Juan Pérez"
   └─→ NIT: "1234567890"
   └─→ Clic en "Nueva Factura"

3. INTERFAZ VALIDA Y LLAMA:
   factura_id = logica.crear_nueva_factura(
       "Juan Pérez", "1234567890", ...
   )

4. LOGICA LLAMA DATABASE:
   factura_id = db.crear_factura(
       cliente_nombre="Juan Pérez",
       cliente_nit="1234567890",
       usuario_id=1
   )

5. DATABASE EJECUTA SQL:
   INSERT INTO facturas (numero_factura, cliente_nombre, ...)
   VALUES ('FAC-20260126-000001', 'Juan Pérez', ...)

6. DATABASE RETORNA factura_id = 5

7. LOGICA RETORNA factura_id = 5

8. INTERFAZ MUESTRA:
   "✓ Factura creada: 5"
   └─→ Permite agregar productos

9. USUARIO AGREGA PRODUCTOS:
   └─→ Selecciona producto
   └─→ Ingresa cantidad
   └─→ Clic "Agregar a Factura"

10. REPITE PASOS 3-8 PARA CADA PRODUCTO

11. USUARIO VISUALIZA FACTURA:
    logica.generar_resumen_factura(5)
    └─→ Obtiene datos factura + items
    └─→ Calcula totales
    └─→ Retorna resumen

12. INTERFAZ MUESTRA FACTURA EN PANTALLA
    └─→ Usuario puede exportar TXT/HTML
```

---

## 🎯 VENTAJAS DE ESTA ARQUITECTURA

### Capa de Interfaz
✅ Separada de la lógica  
✅ Fácil de cambiar (tkinter → PyQt, Web, etc.)  
✅ Solo responsable de UI  

### Capa de Lógica
✅ Reutilizable desde diferentes interfaces  
✅ Reglas de negocio centralizadas  
✅ Fácil de testear  
✅ Independiente de la BD  

### Capa de Datos
✅ Cambio de BD sin afectar lógica (MySQL, PostgreSQL)  
✅ Consultas optimizadas en un lugar  
✅ Transacciones controladas  
✅ Seguridad centralizada  

---

## 📊 ESTADÍSTICAS

| Aspecto | Valor |
|---------|-------|
| Líneas de código | ~1500+ |
| Métodos en LogicaInventario | 25+ |
| Métodos en DatabaseManager | 20+ |
| Tablas en BD | 5 |
| Pestañas en GUI | 5 |
| Funcionalidades | 30+ |

---

## 🔐 SEGURIDAD

| Nivel | Implementado | Futuro |
|-------|-------------|--------|
| Usuario/Contraseña | ✅ | Encriptación bcrypt |
| SQLite Transacciones | ✅ | Auditoría de cambios |
| Validación datos | ✅ | Roles y permisos |
| Inyección SQL | ✅ (Parametrizado) | 2FA |

---

## 🚀 PRÓXIMAS FASES

**FASE 2**: Integración DIAN  
**FASE 3**: Acceso multi-usuario en red  
**FASE 4**: Sincronización en la nube  
**FASE 5**: Aplicación móvil  

---

*Arquitectura escalable, mantenible y profesional para una PYME* 🎯
